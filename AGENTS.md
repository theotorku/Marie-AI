...
Personal AI assistant for beauty industry professionals, powered by Codex.
...
- **AI:** Codex API via server-side proxy (`/api/chat`) + proactive agent (`server/agent.js`)
...
- `src/data/products.ts` — product catalog (static, injected into Codex system prompt + agent prompts)
...
|---|---|---|
| AI Model | Codex Haiku 4.5 | Codex Sonnet 4.6 |
| Messages/day | 20 | 100 |
...
- **Daily Briefing** (8 AM EST) — summarizes calendar, unread emails, open tasks, pipeline status, and stale contacts via Codex
...
- API key never touches the browser — all Codex calls go through Express proxy
...
- Slack slash commands respond immediately with "Thinking..." then post Codex's answer async via response_url (3s Slack timeout)
